...
class SpeckyFormatter < RSpec::Core::Formatters::BaseTextFormatter
    ...
    def first_diff str1, str2
      ...
      str1.each_byte { |c|
        break if c != str2[i]
        pd << i if c == 47 # Stack of all '/' path symbols
        i += 1
      }
      if pd.size > 2
        pd[pd.size() - 2] + 1
	...
	def dump_failures
		...
		@failures.each_with_index do |example, index|
			...
			if RSpec::Core::PendingExampleFixedError === exception
                ...
                self.out "At Backtrace:" 
                max_fd = 0
                first_line = nil
                done = false
                exception.backtrace.each_with_index { |exline, index|
                  trace_line = if exline.start_with? srcfile
                                 done = true
                                 exline[max_fd..-1]
                               else
                                 fd = first_diff(srcfile, exline)
                                 max_fd = [fd, max_fd].max
                                 rspec_pos = exline.index('/lib/rspec')
                                 if !rspec_pos.nil?
                                   nil  # print nothing
                                 elsif fd > 0
                                   exline[fd..-1]
            ...
            self.out "   |  ->  @  #{point_of_failure_file}"
            self.out "   |  ---------------------------------------------------------------------------------"
